package com.learnnow.notes.application;

import com.learnnow.notes.api.dto.BookmarkResponse;
import com.learnnow.notes.api.dto.NoteResponse;
import com.learnnow.notes.persistence.SubtopicNote;
import com.learnnow.notes.persistence.SubtopicNoteRepository;
import com.learnnow.notes.persistence.TopicBookmark;
import com.learnnow.notes.persistence.TopicBookmarkRepository;
...
public class NotesService {
    ...
    public NoteResponse upsertNote(String userId, UUID subtopicId, String content) {
        SubtopicNote note = noteRepository.findByUserIdAndSubtopicId(userId, subtopicId)
                .orElseGet(() -> SubtopicNote.builder()
                        .userId(userId)
    ...
    public Optional<NoteResponse> getNote(String userId, UUID subtopicId) {
        return noteRepository.findByUserIdAndSubtopicId(userId, subtopicId)
    ...
    public boolean toggleBookmark(String userId, UUID topicId) {
        Optional<TopicBookmark> existing = bookmarkRepository.findByUserIdAndTopicId(userId, topicId);
        if (existing.isPresent()) {
            bookmarkRepository.deleteByUserIdAndTopicId(userId, topicId);
            return false; // unbookmarked
        } else {
            TopicBookmark bookmark = TopicBookmark.builder()
                    .userId(userId)
                    .topicId(topicId)
                    .build();
