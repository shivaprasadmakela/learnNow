...
/**
 * Lightweight IP-based rate limiting filter for sensitive endpoints (/api/auth/login,
 * /api/auth/register). Protects against brute-force password guessing and registration spam.
 */
@Component
    ...
    protected void doFilterInternal(
        ...
        if ("POST".equalsIgnoreCase(method)
            ...
            if (tracker.count.get() > MAX_REQUESTS_PER_MINUTE) {
                ...
                response.getWriter()
                        .write(
                                "{\"code\":\"too_many_requests\",\"message\":\"Too many"
                                    + " authentication attempts. Please try again in 1 minute.\"}");
                return;
            }
        }
