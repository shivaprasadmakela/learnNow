...
CREATE TABLE IF NOT EXISTS subtopic_code_snippets (
...
CREATE INDEX IF NOT EXISTS idx_snippets_subtopic ON subtopic_code_snippets(subtopic_id);
...
CREATE INDEX IF NOT EXISTS idx_topic_progress_user_path ON user_topic_progress (user_id, path_id);
...
CREATE INDEX IF NOT EXISTS idx_subtopic_progress_user_topic ON user_subtopic_progress (user_id, topic_id);

-- 11. Create user_learning_daily_activity table
CREATE TABLE IF NOT EXISTS user_learning_daily_activity (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id VARCHAR(255) NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_daily_activity_user_date ON user_learning_daily_activity (user_id, activity_date DESC);
...
CREATE INDEX IF NOT EXISTS idx_subtopic_notes_user ON subtopic_notes (user_id);
...
CREATE INDEX IF NOT EXISTS idx_shared_snippets_short_id ON shared_snippets (short_id);
CREATE INDEX IF NOT EXISTS idx_shared_snippets_code_hash ON shared_snippets (code_hash);

-- 15. Create donation_orders table
